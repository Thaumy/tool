    ...
    namespace ViewHandler
    {
        namespace pilipala
        {
            ...
            public class GetPost : Page, IpalaRoot
            {
                ...
                /// <returns>返回普通文章List postData</returns>
                public List<postData> idx_comnPost(int beginPlace, int length)
                {
                    try
                    {
                        //定义List
                        ...
                        //获得展示可用的文列文章postData表
                        using (MySqlCommand MySqlCommand = new MySqlCommand("select * from " + DataBaseName + ".view_idx_comn_plst LIMIT ?beginPlace , ?length"))
                        {
                            //添加参数
