            ...
            public class GetPost : Page
            {
                ...
                public postData idx_topPost()
                {
                    try
                    {
                        ...
                        using (DataTable postDataTable = MySqlConnectionHandler.getTable("select * from " + DataBaseName + ".view_idx_top_plst"))
                        {
                            foreach (DataRow r in postDataTable.Rows)
                            {
                                //赋值文章列表单文章信息
                ...
                public postData pst_postData(string post_id)
                {
                    try
                    {
                        postData postData = new postData();
                ...
                /// <param name="post_id">文章序列号，用于指定对哪篇文章进行操作</param>
                /// <returns>返回文章数据postData</returns>
                public postData pst_postCount(string post_id)
                {
                    try
                    {
                        postData postData = new postData();
                        ...
                        //建立一个cmd对象，用于执行参数化查询
                        using (MySqlCommand MySqlCommand = new MySqlCommand("select * from " + DataBaseName + ".view_pst_post_count where post_id = ?post_id"))
                        {
                            MySqlCommand.Parameters.AddWithValue("?post_id", post_id);//添加参数
                ...
                /// </summary>
                /// <param name="post_id">文章序列号，用于排除某篇文章以不被随机，该值为小于10000则视为不排除文章随机</param>
                /// <returns>返回随机抓取到的文章数据</returns>
                public DataRow random_post(int post_id)
                {
